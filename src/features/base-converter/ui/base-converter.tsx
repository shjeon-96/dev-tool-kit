  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <Label className={isSource ? "text-primary font-medium" : ""}>
          {label}
...
export function BaseConverter() {
  ...
  const bases: { label: string; base: NumberBase; key: keyof NonNullable<typeof convertedValues> }[] = [
  ...
  const getValue = (base: NumberBase, key: keyof NonNullable<typeof convertedValues>): string => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          아무 진법의 입력 필드에 숫자를 입력하면 자동으로 다른 진법으로 변환됩니다.
        </p>
