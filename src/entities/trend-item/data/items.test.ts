...
describe("trend item data quality", () => {
  ...
  it("keeps every item sourced, dated, and numerically comparable", () => {
    for (const item of trendItems) {
      ...
      expect(item.imageUrl).toBe(
        `/images/trend-battle/generated/${item.id}.webp`,
      );
      expect(item.imageAlt?.length).toBeGreaterThan(8);
  ...
  it("creates a GPT image prompt and path for every trend item", () => {
    for (const item of trendItems) {
      const image = createTrendItemImage(item);

      expect(image.src).toBe(`/images/trend-battle/generated/${item.id}.webp`);
      expect(image.alt).toContain(item.name);
      expect(image.prompt).toContain(item.name);
      expect(image.prompt).toMatch(/Photorealistic/);
      expect(image.prompt).toMatch(/16:9/);
    }
  });
});
