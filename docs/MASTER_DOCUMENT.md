...
**수익화**: Google AdSense (ca-pub-4981986991458105) + Freemium 구독
...
20. [인증 시스템 (Supabase)](#20-인증-시스템-supabase)
21. [결제 시스템 (LemonSqueezy)](#21-결제-시스템-lemonsqueezy)
22. [Premium 및 Quota 시스템](#22-premium-및-quota-시스템)
23. [API v1](#23-api-v1)
...
│   └── comparison/        # 비교 엔티티
│
├── features/              # 도구 및 기능 컴포넌트
│   ├── json-formatter/
│   ├── jwt-decoder/
│   ├── video-compressor/  # 비디오 압축 (Premium)
...
│   │
│   ├── /compare                     # 비교 페이지
│   │   └── /[slug]                  # 개별 비교 (예: md5-vs-sha256)
│   │
│   ├── /convert                     # 변환 조합 페이지 (54개)
│   │   └── /[slug]                  # 개별 변환 (예: json-to-yaml)
│   │
│   ├── /cheatsheets                 # 치트시트 목록
│   │   └── /[slug]                  # 개별 치트시트 (14개)
│   │
...
│   ├── /webhooks/lemonsqueezy       # LemonSqueezy 웹훅
│   ├── /leads/capture               # 리드 캡처
│   └── /v1/                         # Public API v1
│       ├── /hash/generate           # 해시 생성
│       ├── /json/format             # JSON 포맷팅
│       ├── /qr/generate             # QR 코드 생성
│       ├── /uuid/generate           # UUID 생성
│       └── /base64/convert          # Base64 변환
│
├── /robots.txt                      # 검색엔진 로봇
└── /sitemap.xml                     # 사이트맵
```
...
| Text Case Converter | `text-case-converter` | camelCase, snake_case 등 변환     |         |

#### 🔒 보안 (2개)

| 도구           | Slug             | 설명                                     |
| -------------- | ---------------- | ---------------------------------------- |
| JWT Decoder    | `jwt-decoder`    | JWT 토큰 디코딩 및 만료 시간 확인        |
...
Supabase Auth를 사용한 인증 시스템으로, OAuth 소셜 로그인과 이메일 인증을 지원합니다.
...
| **Email/Password** | Supabase Auth  | 이메일 기반 로그인   |
| **Magic Link**     | Supabase Auth  | 비밀번호 없는 로그인 |
...
└── ui/
    ...
    └── oauth-buttons.tsx   # OAuth 버튼 컴포넌트
```
...
## 21. 결제 시스템 (LemonSqueezy)

### 21.1 개요

LemonSqueezy를 사용한 구독 기반 결제 시스템입니다.

### 21.2 요금제

| 티어     | 가격   | 기능                                        |
| -------- | ------ | ------------------------------------------- |
| **Free** | $0/월  | 기본 도구, 일일 50회 제한, 광고 표시        |
| **Pro**  | $10/월 | 모든 도구, 무제한 사용, 광고 제거, API 접근 |
...
├── model/
│   └── use-billing.ts      # 결제 Hook
└── ui/
    └── billing-section.tsx # 결제 관리 UI
...
| 도구               | Slug                 | 설명                         |
| ------------------ | -------------------- | ---------------------------- |
| SVG Optimizer      | `svg-optimizer`      | SVG 파일 최적화              |
| JSON to TypeScript | `json-to-typescript` | JSON → TypeScript 타입 변환  |
| Video Compressor   | `video-compressor`   | FFmpeg.wasm 기반 비디오 압축 |
...
| 도구             | 일일 제한 | 작업 비용 | 비고        |
| ---------------- | --------- | --------- | ----------- |
| video-compressor | 5회       | 5         | 무거운 작업 |
| image-resizer    | 30회      | 2         | 중간 작업   |
| 기타 도구        | 50회      | 1         | 기본값      |
...
| 기능 ID             | 이름              | 설명            |
| ------------------- | ----------------- | --------------- |
| `bulk_actions`      | Bulk Actions      | 다중 파일 처리  |
| `api_access`        | API Access        | REST API 접근   |
| `ad_free`           | Ad-Free           | 광고 제거       |
| `unlimited_history` | Unlimited History | 무제한 히스토리 |
| `priority_support`  | Priority Support  | 우선 지원       |
...
├── index.ts                    # 모듈 export
├── lib/
│   └── feature-gate.ts         # 기능 접근 제어
├── model/
│   ├── types.ts                # 타입 정의
│   └── use-subscription.ts     # 구독 Hook
└── ui/
    ├── plan-badge.tsx          # 요금제 배지
    ├── upgrade-modal.tsx       # 업그레이드 모달
    └── premium-tool-gate.tsx   # 프리미엄 도구 게이트

src/shared/lib/quota/
├── config.ts                   # Quota 설정
├── use-quota.ts                # Quota Hook
└── index.ts                    # 모듈 export
```
...
// 기능 접근 확인
const { canAccess, limit } = useFeatureAccess("bulk_actions");

// Quota 확인
const { remaining, isExceeded } = useQuota("video-compressor");
```
...
Pro 구독자를 위한 REST API입니다. 프로그래매틱하게 도구에 접근할 수 있습니다.
...
```bash
curl -X POST https://web-toolkit.app/api/v1/json/format \
...
API 문서는 `/docs/api` 페이지에서 확인 가능합니다.
...
사용자 대시보드에서 계정 관리, 사용량 확인, API 키 관리를 제공합니다.
...
## 25. 전략적 확장 로드맵 (Strategic Expansion Roadmap)
...
| 항목                     | 우선순위 | 상태    |
| ------------------------ | -------- | ------- |
| Supabase Auth 통합       | 1        | ✅ 완료 |
| LemonSqueezy 결제 시스템 | 2        | ✅ 완료 |
| Quota 시스템             | 3        | ✅ 완료 |
| 가격 페이지              | 4        | ✅ 완료 |
| Dashboard                | 5        | ✅ 완료 |
| API v1                   | 6        | ✅ 완료 |
